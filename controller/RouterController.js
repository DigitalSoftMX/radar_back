            ...
            if (getToday2 != getToday) {
                ...
                for (let i = 0; i < datePurchase.length; i++) {
                    ...
                    if (req.body.purchaseRegular.date == dateRow) {
                        console.log(req.body.purchaseRegular.date, dateRow)
                        let x = await Purchase.findOneAndUpdate({'createdAt': dateRow},{'purchaseRegular_price':req.body.purchaseRegular.price})
                        console.log(x)
                    }
/*                    if(req.body.purchasePremium.date == dateRow){
                        await Purchase.findOneAndUpdate({'createdAt': dateRow},{'purchasePremium_price':req.body.purchasePremium.price})
                    }
                    if(req.body.purchaseDiesel.date = dateRow){
                        await Purchase.findOneAndUpdate({'createdAt': dateRow},{'purchaseDiesel_price':req.body.purchaseDiesel.price})
                    }
                    if(req.body.recommendedRegular.date = dateRow){
                        await Purchase.findOneAndUpdate({'createdAt': dateRow},{'recommendedPremium_price':req.body.recommendedRegular.price})
                    }
                    if(req.body.recommendedPremium.date = dateRow){
                        await Purchase.findOneAndUpdate({'createdAt': dateRow},{'recommendedRegular_price':req.body.recommendedPremium.price})
                    }
                    if(req.body.recommendedDiesel.date = dateRow){
                        await Purchase.findOneAndUpdate({'createdAt': dateRow},{'recommendedDiesel_price':req.body.recommendedDiesel.price})
                    } else {
                        console.log('nada :C');
                    }  */
